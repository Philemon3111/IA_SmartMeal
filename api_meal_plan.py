...
categories = {
    ...
    "arômes et extraits": ["Arôme d'Amandes", "Arôme d'orange", "Arôme de Citrons", "Arôme de courge musquée", "Arôme de noix de beurre", "Arôme de noix de coco", "Arôme de rhum", "Arôme de vanille", "Extrait d'Amandes", "Extrait d'orange", "Extrait de Citrons", "Extrait de rhum", "Extrait de vanille", "Fumée liquide", "Vanille", "Bouillon de bœuf", "Bouillon de porc", "Bouillon de poulet", "Assaisonnement pour pizza", "Assaisonnement pour salade"],
    ...
    "emballages alimentaires": ["Coquilles", "Croûte à pizza", "Croûtes à tarte", "Croûtons", "Fonds de tarte"],
...
def flatten_quantities(extracted):
    ...
    for category, items in extracted.items():
        ...
        for name, qty in item_counts.items():
            ...
            elif category in ["huiles", "sauces", "boissons", "produits laitiers liquides (ml)"]:
    ...
    for category, items in flattened.items():
        for item_str in items:
            ...
            if category not in no_unit:
                ...
                if remaining_qty > 0:
                    if category in ["huiles", "produits laitiers liquides (ml)", "sauces", "boissons"]:
                        final_list[category].append(f"{name}: {int(remaining_qty // 250) + (1 if remaining_qty % 1000 != 0 else 0)} bouteille")
                    else:
                        if name == "Œuf":
                            egg_count = int(remaining_qty/50)
                            boxes_30 = egg_count // 30
                            egg_count %= 30
                            boxes_12 = egg_count // 12
                            egg_count %= 12
                            boxes_6 = int(egg_count / 6)  # si restants < 6, on prend une boîte de 6
                            parts = []
                            if boxes_30 > 0:
                                parts.append(f"{boxes_30} boîte(s) de 30")
                            if boxes_12 > 0:
                                parts.append(f"{boxes_12} boîte(s) de 12")
                            if boxes_6 > 0:
                                parts.append(f"{boxes_6} boîte(s) de 6")

                            final_list[category].append(f"{name}: {', '.join(parts)}")
                        elif name == "Eau":
                            continue
                        elif remaining_qty>1000 and unit == "g":
                            final_list[category].append(f"{name}: {int(remaining_qty/1000) + (1 if remaining_qty % 1000 != 0 else 0)} kg")
                        else:
                            final_list[category].append(f"{name}: {int(remaining_qty) + (1 if remaining_qty % 1 != 0 else 0)} {unit}")
            else:
